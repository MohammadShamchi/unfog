...
import { generateRoughRectangle } from "@/lib/sketch/rough-shapes";
...
function ContextNodeComponent({ id, data, selected }: NodeProps<InsightNode>) {
  const { width, height } = getShapeDimensions("context");
  const color = SHAPE_STROKE_COLORS.context;
  ...
  const {
    ...
    reducedMotion,
    isDark,
  } = useNodeInteraction(id, "context");

  const delay = data.animationDelay ?? 0;

  const shape = useMemo(
    () => generateRoughRectangle(width, height, id),
    [width, height, id],
  );

  const fillColor = isDark ? SHAPE_FILL_DARK.context : SHAPE_FILL_LIGHT.context;
  const hoverFill = isDark ? "rgba(123, 135, 148, 0.08)" : "rgba(123, 135, 148, 0.12)";
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ width, height, padding: "12px 20px" }}
      >
        <NodeContent
          nodeId={id}
          data={data}
