...
import { generateRoughDiamond } from "@/lib/sketch/rough-shapes";
...
function SolutionNodeComponent({ id, data, selected }: NodeProps<InsightNode>) {
  const { width, height } = getShapeDimensions("solution");
  const color = SHAPE_STROKE_COLORS.solution;
  ...
  const {
    ...
    reducedMotion,
    isDark,
  } = useNodeInteraction(id, "solution");

  const delay = data.animationDelay ?? 0;

  const shape = useMemo(
    () => generateRoughDiamond(width, height, id),
    [width, height, id],
  );

  const fillColor = isDark ? SHAPE_FILL_DARK.solution : SHAPE_FILL_LIGHT.solution;
  const hoverFill = isDark ? "rgba(93, 202, 165, 0.08)" : "rgba(93, 202, 165, 0.12)";
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ width, height, padding: "16px 40px" }}
      >
        <NodeContent
          nodeId={id}
          data={data}
